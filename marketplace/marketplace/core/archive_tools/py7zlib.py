...
class ArchiveFile(Base):
    ...
    def read(self, chunks_callback=None):
        if not self.size:
            return ''
        ...
        for level, coder in enumerate(self._folder.coders):
            ...
            data = getattr(self, decoder)(coder, data, level, num_coders, chunks_callback)

        return data
    
    def _read_copy(self, coder, input, level, num_coders, chunks_callback):
        size = self._uncompressed[level]
        if not input:
            self._file.seek(self._src_start)
            input = self._file.read(size)
        result = input[self._start:self._start+size]
        if chunks_callback:
            chunks_callback(len(result))
        return result

    def _read_delta(self, coder, input, level, num_coders, chunks_callback):
        size = self._uncompressed[level]
        if not input:
            self._file.seek(self._src_start)
            input = self._file.read(size)
        ...
        result = data[self._start:self._start+size]
        if chunks_callback:
            chunks_callback(len(result))
        return result

    def _read_from_decompressor(self, coder, decompressor, input, level, num_coders, can_partial_decompress=True, with_cache=False, chunks_callback=None):
        size = self._uncompressed[level]
        data = ''
        idx = 0
        cnt = 0
    ...
    def _read_lzma(self, coder, input, level, num_coders, chunks_callback):
        size = self._uncompressed[level]
        is_last_coder = (level + 1) == num_coders
        ...
        try:
            return self._read_from_decompressor(coder, dec, input, level, num_coders, with_cache=True, chunks_callback=chunks_callback)
        except ValueError:
    ...
    def _read_lzma2(self, coder, input, level, num_coders, chunks_callback):
        size = self._uncompressed[level]
        is_last_coder = (level + 1) == num_coders
        if is_last_coder and not self._folder.solid:
    ...
    def _read_zip(self, coder, input, level, num_coders, chunks_callback):
        dec = zlib.decompressobj(-15)
        return self._read_from_decompressor(coder, dec, input, level, num_coders, chunks_callback=chunks_callback)
        
    def _read_bzip(self, coder, input, level, num_coders):
        dec = bz2.BZ2Decompressor()
        return self._read_from_decompressor(coder, dec, input, level, num_coders, can_partial_decompress=False, chunks_callback=chunks_callback)
    
    def _read_7z_aes256_sha256(self, coder, input, level, num_coders, chunks_callback):
    ...
    def _read_bcj2(self, coder, input, level, num_coders, chunks_callback):
        size = self._uncompressed[level]
        if not input:
            self._file.seek(self._src_start)
            input = self._file.read(size)
        ...
        result = pylzma.bcj2_decode(main_data, call_data, jump_data, rc_data, self.uncompressed)
        if chunks_callback:
            chunks_callback(len(result))
        return result

    def _read_bcj(self, coder, input, level, num_coders, chunks_callback):
        size = self._uncompressed[level]
        if not input:
            self._file.seek(self._src_start)
            input = self._file.read(size)
    ...
    def _read_bcj_ppc(self, coder, input, level, num_coders, chunks_callback):
        size = self._uncompressed[level]
        if not input:
            self._file.seek(self._src_start)
            input = self._file.read(size)
    ...
    def _read_bcj_ia64(self, coder, input, level, num_coders, chunks_callback):
        size = self._uncompressed[level]
        if not input:
            self._file.seek(self._src_start)
            input = self._file.read(size)
    ...
    def _read_bcj_arm(self, coder, input, level, num_coders, chunks_callback):
        size = self._uncompressed[level]
        if not input:
            self._file.seek(self._src_start)
            input = self._file.read(size)
    ...
    def _read_bcj_armt(self, coder, input, level, num_coders, chunks_callback):
        size = self._uncompressed[level]
        if not input:
            self._file.seek(self._src_start)
            input = self._file.read(size)
    ...
    def _read_bcj_sparc(self, coder, input, level, num_coders, chunks_callback):
        size = self._uncompressed[level]
        if not input:
            self._file.seek(self._src_start)
            input = self._file.read(size)
    ...
    def _read_ppmd(self, coder, input, level, num_coders, chunks_callback):
        size = self._uncompressed[level]
        if not input:
            self._file.seek(self._src_start)
            input = self._file.read(self.compressed)
